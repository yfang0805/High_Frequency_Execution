...
class DefaultConfig(object):
    ...
    result_path = 'results/dqn_CSI300'

    
    code_list = CODE_LIST_CSI300
    date_list = DATE_LIST_202312
    code_list_validation = CODE_LIST_CSI300
    date_list_validation = DATE_LIST_202312_Validation
...
class Agent(object):
    ...
    #         # Include other necessary items here
    #     }
    #     torch.save(state, checkpoint_path)

    # def load(self, checkpoint_path):
    #     state = torch.load('model_and_optimizer.pth')
    ...
    def evaluate(self, agent):
        ...
        for code in self.config.code_list_validation:
            for date in self.config.date_list_validation:
                if not self.env.data.data_exists(code, date):
                    print(f"Data Not Exist: code_{code}, date_{date}")
                    continue
...
def test_dqn_no_mp():
    ...
    record = run((model, lr, lin_reg, parrallel_id))
...
def test_dqn_with_mp():
    record = []
    test_list = list(itertools.product(
        [MLPNetwork_complex, MLPNetwork_complex], 
        [2e-5, 1e-5, 5e-6], 
        [0.1, 0.01], 
        np.arange(5)
    ))
